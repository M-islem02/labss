    ...
    import { LAB_MODES, LAB_QUERY_ALIASES, TRANSLATIONS } from './lab-legacy-data.js';
    ...
    function t(key) {
      return getTranslationValue(currentLanguage, key) ?? getTranslationValue('ar', key) ?? key;
    }
    ...
    function setLabMode(mode) {
      ...
      document.body.classList.toggle('mode-unity', mode === 'unity');
      ...
      unityUi.shell.setAttribute('aria-hidden', String(mode !== 'unity'));
    ...
    buildLab()
      .then(() => {
        ...
        const requestedLabParam = new URLSearchParams(window.location.search).get('lab');
        const requestedLabMode = normalizeLabMode(requestedLabParam);
